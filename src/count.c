...
/*   Updated: 2020/02/03 15:48:46 by mvo-van-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_count(t_graph *graph)
{
	...
	total = 0;
	while (--i >= 0 && (graph->count_next_paths == -1 ||
		graph->count_next_paths > graph->next_paths[t[i]].size))
	{
...
void		ft_distrib_f(t_graph *graph)
{
	...
	while (--i >= 0 && nb_f)
	{
		if ((graph->count_curr_paths -
			(*graph).curr_paths[t[i]].size + 1) >= nb_f)
			(*graph).curr_paths[t[i]].f = nb_f;
		else
			(*graph).curr_paths[t[i]].f =
				graph->count_curr_paths - (*graph).curr_paths[t[i]].size + 1;
